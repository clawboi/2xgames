    ...
    if (this.hp <= 0) {
      ...
      if (game) {
        ...
        for (let i = 0; i < gibCount; i++) {
          ...
          if (game.gibs.length < 80) game.gibs.push(new Gib(this.x, this.y, col, sz));
        }
  ...
  update(dt, game) {
    ...
    if (this.tidalSlamTelegraph > 0) {
      ...
      if (this.tidalSlamTelegraph <= 0) {
        ...
        game.spawnExplosion(this.x, this.y, '#0099ff', 50);
        try { Audio.sfx.explode(); } catch (e) {}
        const dp = Math.hypot(game.player.x - this.x, game.player.y - this.y);
        if (dp < pulseRadius) {
          game.player.damage(30, game);
          // Knockback
          const ang = Math.atan2(game.player.y - this.y, game.player.x - this.x);
          game.player.x += Math.cos(ang) * 30;
          game.player.y += Math.sin(ang) * 30;
        }
        this.tidalSlamCooldown = 6500;
      }
    } else if (this.tidalSlamCooldown <= 0) {
      this.tidalSlamTelegraph = 900;
...
class Slimey extends Enemy {
  ...
  update(dt, game) {
    ...
    // MIC TOSS — every 5s throws 2 mics that arc + deal heavier damage
    this.micTossCooldown = (this.micTossCooldown || 5000) - dt;
    if (this.micTossCooldown <= 0) {
      this.micTossCooldown = 5500;
      game.spawnFloater(this.x, this.y - 50, 'MIC DROP', '#ff00aa', 16, -0.3);
      for (let i = 0; i < 2; i++) {
        const ang = Math.atan2(this.target.y - this.y, this.target.x - this.x) + (i === 0 ? -0.35 : 0.35);
        const b = new Bullet(this.x, this.y, Math.cos(ang) * 7, Math.sin(ang) * 7, 22, 'enemy', 'enemy', { splash: 50, explodeOnExpire: true });
        b.life = 1400;
        game.bullets.push(b);
      }
...
class Mirror2X extends Enemy {
  ...
  update(dt, game) {
    ...
    // CLONE SUMMON — at <50% HP, summons 1-2 mirror clones (FastCrabs with red tint via cust)
    if (this.hp / this.maxHp < 0.5) {
      this.cloneCooldown = (this.cloneCooldown || 6000) - dt;
      if (this.cloneCooldown <= 0) {
        this.cloneCooldown = 7500;
        game.spawnFloater(this.x, this.y - 60, 'CLONES', '#ff00ff', 18, -0.3);
        for (let i = 0; i < 2; i++) {
          const ang = Math.random() * Math.PI * 2;
          const minion = new FastCrab(this.x + Math.cos(ang) * 70, this.y + Math.sin(ang) * 70, 1.2);
...
class Follower {
  ...
  update(dt, game) {
    ...
    // In party, dance in formation around player
    if (game.state === 'party') {
    ...
    // Otherwise — trail player with delay, offset per index
  ...
  draw(ctx) {
    ...
    Sprites.drawFollower(ctx, this.x, this.y + bob, this.frame + this.danceStyle * 10, this.dir, this.color);
  }
}
