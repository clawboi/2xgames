...
class Player {
  ...
  update(dt, game) {
    ...
    // Movement — use network input if set (for coop player2)
    const moveSrc = this.inputSource;
    const move = moveSrc ? { x: moveSrc.mx || 0, y: moveSrc.my || 0 } : Input.getMoveVector();
    let speed = this.speed * this.speedMultiplier;

    // RAGE mode — AOE chomp every 200ms damaging nearby crabs, ends after rageEnd
    if (this.rageEnd && now < this.rageEnd) {
      this._lastChomp = this._lastChomp || 0;
      if (now - this._lastChomp > 200) {
        this._lastChomp = now;
        const chompR = 60;
        let chomped = 0;
        for (const e of game.enemies) {
          ...
          if (d < chompR) {
            try { e.damage(35 * this.damageMultiplier, game, false); } catch (er) {}
            game.spawnSparks(e.x, e.y, '#ff3300', 4);
            chomped++;
          }
        }
        if (chomped > 0) {
          try { Audio.sfx.crit && Audio.sfx.crit(); } catch (er) {}
          game.addScreenShake(2);
        }
      }
    } else if (this.rageEnd && now >= this.rageEnd) {
      // Restore weapon when RAGE ends
      if (typeof this._prevWeaponIdx === 'number') {
        this.weaponIdx = this._prevWeaponIdx;
        this._prevWeaponIdx = null;
      }
      this.rageEnd = null;
      game.spawnFloater(this.x, this.y - 30, 'RAGE OVER', '#aaaaaa', 14, -0.3);
    }
    if (this.dashTime > 0) speed *= 3.0;
    ...
    if (Input.consumeDash() && this.dashCooldown <= 0) {
      this.dashTime = 270;
      this.dashCooldown = 1500;  // shorter so you can use it more — was 2200
      try { Audio.sfx.pickup(); } catch (e) {}
      game.spawnSparks(this.x, this.y, '#00ff66', 12);
      game.slowMo(420, 0.22);
      // Mark all currently-near enemies so they take dash damage if touched during dash
      this._dashHitSet = new Set();
    }
    // Dash-through damage: any enemy you touch during dash takes 35 damage + gets knocked back
    if (this.dashTime > 0 && game.enemies) {
      for (const e of game.enemies) {
        if (e.dead || this._dashHitSet.has(e)) continue;
        const d = Math.hypot(e.x - this.x, e.y - this.y);
        if (d < (e.radius || 12) + 18) {
          this._dashHitSet.add(e);
          try { e.damage(35 * this.damageMultiplier, game, false, null); } catch (er) {}
          game.spawnSparks(e.x, e.y, '#00ff66', 4);
          // Push enemy in dash direction (away from player along velocity)
          const dashAng = Math.atan2(this.vy, this.vx);
          e.knockX += Math.cos(dashAng) * 8;
          e.knockY += Math.sin(dashAng) * 8;
        }
      }
    }

    // Aiming — touch device OR hold SPACE on desktop = auto-aim
    // COOP: networked player (player2 with inputSource) ALWAYS auto-aims so
    //       cross-platform pairs (desktop+iPhone) both work correctly
    if (this.inputSource || Input.isTouchDevice() || Input.isKey(' ')) {
      ...
      if (lock) {
        ...
        // No target — face the direction you're MOVING (so shots spray where you turn)
        let targetAng = this.facing;
        const moveLen = Math.hypot(this.vx || 0, this.vy || 0);
        if (moveLen > 0.1) {
          targetAng = Math.atan2(this.vy, this.vx);
        }
        let diff = targetAng - this.facing;
        while (diff > Math.PI) diff -= Math.PI * 2;
        while (diff < -Math.PI) diff += Math.PI * 2;
    ...
    const fireSrc = this.inputSource;
    const isFiring = fireSrc ? !!fireSrc.fire : Input.isFiring();
    // Auto-aim if locked, otherwise shoot in last facing direction (no longer suppress)
    if (isFiring
        && this.gunLockEnd <= now
        && this.emoteEnd <= now
        && game.state !== 'party'
    ...
    const wantReload = fireSrc ? !!fireSrc.reload : Input.isKey('r');
    if (wantReload) this.reload();

    // Dash from network input (consume one-shot)
    if (fireSrc && fireSrc.dash && this.dashCooldown <= 0) {
      this.dashTime = 270;
      this.dashCooldown = 1500;
      this._dashHitSet = new Set();
      fireSrc.dash = false; // consume
    }

    // Net-input weapon switch
    if (fireSrc && typeof fireSrc.wep === 'number') {
      if (this.equippedWeapons.has(fireSrc.wep)) this.weaponIdx = fireSrc.wep;
    }

    // (No aim override — networked players use auto-aim via the block above)
  ...
  draw(ctx) {
    ...
    if (this.deathTilt) {
      ...
      ctx.rotate(this.deathTilt);
      ctx.translate(-this.x, -this.y);
    }
    Sprites.drawPlayer(ctx, this.x, this.y, this.cust, this.dir, this.emoteEnd > now ? (Math.floor(now / 80) % 4) : this.frame);
    if (this.deathTilt) ctx.restore();

    // COOP: nameplate + HP bar above head
    if (this._slot !== undefined || this._coopLabel) {
      const slot = this._slot || 0;
      const tints = ['#ff0033', '#00aaff', '#ffcc00', '#00ff66'];
      const tint = tints[slot] || '#fff';
      const label = this._coopLabel || ('P' + (slot + 1));
      ctx.save();
      ...
      ctx.shadowBlur = 0;
      ctx.fillStyle = '#000';
      ctx.fillRect(this.x - barW/2 - 1, this.y - 22, barW + 2, barH + 2);
      ctx.fillStyle = tint;
      ctx.fillRect(this.x - barW/2, this.y - 21, barW * pct, barH);
      ctx.restore();
    }
    ...
    if (this.muzzleFlashTime > 0 && this.emoteEnd <= now && this.gunLockEnd <= now) {
      ...
      const curWep = Weapons.get(this.weaponIdx);
      Sprites.drawMuzzleFlash(ctx, fx, fy, this.facing, curWep.tier);
    }

    // Auto-aim target reticle — rotating crosshair, color reflects target HP
    if (this._aimLock && !this._aimLock.dead && (typeof window !== 'undefined' && (this._aimLock._isLocked = true))) {
      const t = this._aimLock;
      const r = (t.radius || 12) + 8;
      const pulse = 0.7 + Math.sin(now / 100) * 0.3;
      const hpFrac = Math.max(0, (t.hp || 0) / (t.maxHp || 1));
      // Color shifts red→yellow→white as the target weakens
      const col = hpFrac > 0.66 ? '#ff3344' : hpFrac > 0.33 ? '#ffcc00' : '#ffffff';
      ctx.save();
      ctx.translate(t.x, t.y);
      // Slow continuous rotation for tactical feel
      ctx.rotate((now / 600) % (Math.PI * 2));
      ctx.strokeStyle = col;
      ctx.lineWidth = 1.5;
      ctx.globalAlpha = pulse;
      // 4 rotating ticks at compass points
      ctx.beginPath();
...
class Bullet {
  ...
  update(dt, game) {
    ...
    this.y += this.vy;
    this.life -= dt;
    // Range-limited bullets (vocal shockwave): die after traveling `range` px
    if (this.range) {
      this._traveled = (this._traveled || 0) + Math.hypot(this.vx, this.vy);
      if (this._traveled >= this.range) { this.dead = true; return; }
    }
  ...
  draw(ctx) {
    if (this._isVocal) {
      // Big magenta shockwave glyph
      const r = 9 + Math.sin(performance.now() / 40) * 2;
      ctx.save();
      ctx.fillStyle = 'rgba(255,68,255,0.45)';
...
class Enemy {
    ...
    if (this.hp <= 0) {
      ...
      if (game) {
        ...
        if (this.isBoss) {
          ...
          if (game.coopActive && game.coopRole === 'host' && typeof Coop !== 'undefined') {
            try { Coop.broadcast('sfx', { name: 'bossDown' }); } catch (e) {}
          }
  ...
  chooseTarget(game) {
    // Pick the nearest player (works in solo + coop)
    const np = game.findNearestPlayer ? game.findNearestPlayer(this.x, this.y) : game.player;
    if (game.arenaActive) { this.target = np; return; }
    const dPlayer = Math.hypot(np.x - this.x, np.y - this.y);
    this.target = dPlayer < 100 ? np : game.truck;
  }
  applyKnockback() {
    this.x += this.knockX;
    this.y += this.knockY;
  ...
  moveToward(tx, ty, mult = 1) {
    ...
    if (this.frozenUntil && performance.now() < this.frozenUntil) { this.frozen = true; this.vx = 0; this.vy = 0; return; }
    this.frozen = false;
    // Honey slow — multiply mult by slowMult while active
    if (this.slowUntil && performance.now() < this.slowUntil) {
      mult *= (this.slowMult || 0.5);
    }
    const dx = tx - this.x, dy = ty - this.y;
    const dist = Math.hypot(dx, dy);
    if (dist < 1) return;
    const ts = (typeof window !== 'undefined' && window.game && window.game.timeScale) || 1;
    const sp = this.speed * mult * (this.stunTime > 0 ? 0.3 : 1) * ts;
    this.vx = (dx / dist) * sp;
    this.vy = (dy / dist) * sp;
    this.x += this.vx;
    this.y += this.vy;
...
class TankCrab extends Enemy {
  constructor(x, y, mod = 1) {
    super(x, y, { hp: 150 * mod, speed: 0.7, damage: 16, radius: 18, score: 30 });
    this.frame = Math.floor(Math.random() * 100);
    this.scale = 1.5;
  }
  update(dt, game) {
...
class ExploderCrab extends Enemy {
  ...
  onDeath(game) {
    ...
    const np = game.findNearestPlayer ? game.findNearestPlayer(this.x, this.y) : game.player; const dp = Math.hypot(np.x - this.x, np.y - this.y);
...
class GiantCrab extends Enemy {
  ...
  update(dt, game) {
    ...
    // SECOND-ROUND ONLY: ARMOR SHELL — every 6s, locks down for 1.5s with 75% damage reduction
    if (this.isSecondRound) {
      this.armorCooldown = (this.armorCooldown !== undefined ? this.armorCooldown : 6000) - dt;
      if (this.armoredUntil && performance.now() < this.armoredUntil) {
        this.armorActive = true;
      } else {
        this.armorActive = false;
      }
      if (this.armorCooldown <= 0 && !this.armorActive) {
        this.armorCooldown = 6500;
        this.armoredUntil = performance.now() + 1500;
        game.spawnFloater(this.x, this.y - 70, '🛡 ARMORED', '#ffcc00', 16, -0.3);
        try { Audio.sfx.boss(); } catch (e) {}
      }
    }
  }
  damage(amount, game, isCrit, source) {
    // Armor shell reduces incoming damage by 75%
    if (this.armorActive) amount *= 0.25;
    return super.damage(amount, game, isCrit, source);
  }
  draw(ctx) {
    Sprites.drawGiantCrab(ctx, this.x, this.y, this.frame, this.hp / this.maxHp, this.hurtFlash > 0);
    // Armor shell glow (second-round only)
    if (this.armorActive) {
      const pulse = 0.5 + Math.sin(this.frame * 0.4) * 0.3;
...
class Slimey extends Enemy {
  ...
  update(dt, game) {
    ...
    // SECOND-ROUND ONLY: TOXIC POOL — drops a persistent damage zone every 4s
    if (this.isSecondRound) {
      this.toxicPoolCooldown = (this.toxicPoolCooldown !== undefined ? this.toxicPoolCooldown : 4000) - dt;
      if (this.toxicPoolCooldown <= 0) {
        this.toxicPoolCooldown = 4000;
        // Drop a pool at boss's position
        game.toxicPools = game.toxicPools || [];
        game.toxicPools.push({
          x: this.x, y: this.y, radius: 70,
          life: 7000, // 7 seconds
          damage: 8, // per tick
          lastTick: 0,
        });
        game.spawnFloater(this.x, this.y - 50, '☢ TOXIC', '#00ff00', 14, -0.3);
        try { Audio.sfx.hurt && Audio.sfx.hurt(); } catch (e) {}
      }
    }
  }
...
class Mirror2X extends Enemy {
  ...
  update(dt, game) {
    ...
    if (this.stunTime > 0) this.stunTime -= dt;
    this.applyKnockback();
    // Target nearest player (works in coop too)
    this.target = game.findNearestPlayer ? game.findNearestPlayer(this.x, this.y) : game.player;
    this.moveToward(this.target.x, this.target.y, 1.0);
    this.shootCooldown -= dt;
    this.dashCooldown -= dt;
    this.frame++;
    ...
    // CLONE SUMMON — at <70% HP, summons 1-2 fast crabs every 5s
    ...
    // PHASE 2 — past 50% HP, MAJOR escalation
    if (this.hp / this.maxHp < 0.5) {
      // One-time spawn: 9 mirror clones (FastCrabs styled to be lethal)
      if (!this.phase2Triggered) {
        this.phase2Triggered = true;
        game.spawnFloater(this.x, this.y - 80, 'I AM LEGION', '#ff00ff', 32, -0.4, 2500);
        game.spawnFloater(this.x, this.y - 50, '— THE 9 RETURN —', '#ffaa00', 16, -0.3, 2500);
        game.addScreenShake(20);
        try { Audio.sfx.boss(); } catch (e) {}
        // Spawn 9 mirror clones in a ring
        for (let i = 0; i < 9; i++) {
          const ang = (i / 9) * Math.PI * 2;
          const cx = this.x + Math.cos(ang) * 120;
          const cy = this.y + Math.sin(ang) * 120;
          const clone = new FastCrab(cx, cy, 1.8);
          clone.isMirrorClone = true;
          // Clones can shoot back at the player
          clone.shootCooldown = 1200 + Math.random() * 800;
          game.enemies.push(clone);
        }
        // Visual ring explosion
        for (let i = 0; i < 36; i++) {
          const a = (i / 36) * Math.PI * 2;
          game.spawnSparks(this.x + Math.cos(a) * 24, this.y + Math.sin(a) * 24, '#ff00ff', 4);
        }
      }
      // VOCAL SHOCKWAVE — fast directional cone every ~3.5s past 50%
      this.shockwaveCooldown = (this.shockwaveCooldown !== undefined ? this.shockwaveCooldown : 3500) - dt;
      if (this.shockwaveCooldown <= 0) {
        this.shockwaveCooldown = 3500;
        // Telegraph: brief glow, then strike
        game.spawnFloater(this.x, this.y - 40, '〘 VOCAL 〙', '#ff66ff', 16, -0.4, 700);
        const targetAng = Math.atan2(this.target.y - this.y, this.target.x - this.x);
        this._vocalAng = targetAng;
        this._vocalLockIn = performance.now() + 600; // 600ms wind-up
      }
      // Strike timing
      if (this._vocalLockIn && performance.now() >= this._vocalLockIn) {
        this._vocalLockIn = null;
        const ang = this._vocalAng;
        // Spawn a vocal cone "shockwave bullet": fast, narrow, ~280px range
        const SHOCKWAVE_RANGE = 280;
        const speed = 16;
        // Draw burst of sound-wave particles in cone
        for (let i = -2; i <= 2; i++) {
          const a = ang + i * 0.18;
          const vx = Math.cos(a) * speed, vy = Math.sin(a) * speed;
          const b = new Bullet(this.x, this.y, vx, vy, 28, 'enemy', 'enemy', {});
          b.range = SHOCKWAVE_RANGE;
          b._isVocal = true;
          b.color = '#ff44ff';
          game.bullets.push(b);
        }
...
class PowerUp {
  ...
  update(dt, game) {
    ...
    // Magnet to NEAREST player (coop-aware)
    const np = game.findNearestPlayer ? game.findNearestPlayer(this.x, this.y) : game.player;
    const dx = np.x - this.x;
    const dy = np.y - this.y;
    const d = Math.hypot(dx, dy);
    if (d < 70) {
      this.x += (dx / d) * 3;
      this.y += (dy / d) * 3;
    }
    if (d < (np.radius || 12) + this.radius) {
      this.apply(game, np);
      this.dead = true;
    }
  }
  apply(game, targetPlayer) {
    const p = targetPlayer || game.player;
    try { Audio.sfx.pickup(); } catch (e) {}
    let label = '';
    switch (this.type) {
      case 'health':
...
class Lightning {
  ...
  update(dt, game) {
    ...
    if (!this.struck && this.elapsed >= this.telegraphMs) {
      ...
      const np = game.findNearestPlayer ? game.findNearestPlayer(this.x, this.y) : game.player; const dp = Math.hypot(np.x - this.x, np.y - this.y);
      if (dp < this.radius) np.damage(this.damage, game);
      for (const e of game.enemies) {
        if (e.dead) continue;
        const d = Math.hypot(e.x - this.x, e.y - this.y);
        if (d < this.radius) {
