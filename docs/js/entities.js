  ...
  tryFire(game) {
    ...
    for (let i = 0; i < w.shotsPerFire; i++) {
      ...
      game.bullets.push(new Bullet(
        ...
        w.damage * dmgMult,
        w.bulletType, 'player',
        { splash: w.splash || 0, pierce: !!w.pierce, explodeOnExpire: !!w.explodeOnExpire, life: w.range || 650 }
      ));
    }
...
class Bullet {
  constructor(x, y, vx, vy, damage, type, source, opts = {}) {
    ...
    this.source = source;
    this.dead = false;
    this.life = opts.life || 650;  // shorter range so guns aren't infinite-range
