...
class Player {
  ...
  update(dt, game) {
    ...
    // When auto-aiming with no target, suppress fire (was: fired straight into nothing)
    const autoAiming = this.inputSource || Input.isTouchDevice() || Input.isKey(' ');
    const canFire = !autoAiming || !!this._aimLock;
    if (isFiring && canFire
        && this.gunLockEnd <= now
        && this.emoteEnd <= now
        && game.state !== 'party'
