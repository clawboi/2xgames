...
class Player {
  ...
  update(dt, game) {
    ...
    if (Input.consumeDash() && this.dashCooldown <= 0) {
      ...
      // Dash → 500ms of crisp slo-mo for cinematic feel
...
class Enemy {
  ...
  damage(amt, game, isCrit = false, bullet = null) {
    ...
    this.hurtFlash = 150;
    if (bullet) {
      // Bosses resist knockback heavily so they can finish their power moves
...
class Crab extends Enemy {
  constructor(x, y, mod = 1) {
    super(x, y, { hp: 50 * mod, speed: 1.0, damage: 8, radius: 12, score: 10 });
...
class GiantCrab extends Enemy {
  constructor(x, y, hpMult = 1) {
    super(x, y, { hp: 2400 * hpMult, speed: 0.7, damage: 28, radius: 50, score: 800 });
    this.isBoss = true;
...
class Slimey extends Enemy {
  constructor(x, y, hpMult = 1) {
    super(x, y, { hp: 3600 * hpMult, speed: 1.0, damage: 24, radius: 32, score: 1500 });
    this.isBoss = true;
    this.name = '2SLIMEY';
    this.frame = 0;
  ...
  update(dt, game) {
    ...
    if (this.micTossCooldown <= 0) {
      ...
      for (let i = -1; i <= 1; i++) {
        ...
        const b = new Bullet(this.x, this.y, Math.cos(ang) * 7, Math.sin(ang) * 7, 26, 'enemy', 'enemy', { splash: 60, explodeOnExpire: true });
        b.life = 1500;
        game.bullets.push(b);
      }
...
class Mirror2X extends Enemy {
  constructor(x, y, customization, hpMult = 1) {
    super(x, y, { hp: 5200 * hpMult, speed: 1.6, damage: 32, radius: 28, score: 3000 });
    this.isBoss = true;
    this.name = 'MIRROR 2X';
    this.cust = customization;
    this.frame = 0;
