...
(function () {
  ...
  function render() {
    ...
    // DANCE FLOOR — center area with strobing colors
    const t = performance.now() / 200;
    const dfX = W / 2 - 100, dfY = H / 2 - 60, dfW = 200, dfH = 120;
    for (let y = dfY; y < dfY + dfH; y += 30) {
      for (let x = dfX; x < dfX + dfW; x += 30) {
        const hue = (Math.sin(t + (x + y) * 0.05) * 0.5 + 0.5) * 360;
        ctx.fillStyle = `hsla(${hue}, 80%, 50%, 0.4)`;
        ctx.fillRect(x, y, 30, 30);
      }
    }
    // DJ BOOTH — top-center red counter with speakers
    const dbX = W / 2 - 70, dbY = 22, dbW = 140, dbH = 30;
    ctx.fillStyle = '#330011';
    ctx.fillRect(dbX, dbY, dbW, dbH);
    ctx.fillStyle = '#ff0033';
    ctx.fillRect(dbX, dbY, dbW, 5);
    // Speakers
    ctx.fillStyle = '#222';
    ctx.fillRect(dbX - 16, dbY, 12, dbH);
    ctx.fillRect(dbX + dbW + 4, dbY, 12, dbH);
    // Speaker pulse with music
    const pulse = 4 + Math.sin(t * 2) * 2;
    ctx.fillStyle = `rgba(255,0,51,${0.4 + Math.sin(t * 2) * 0.3})`;
    ctx.beginPath(); ctx.arc(dbX - 10, dbY + dbH / 2, pulse, 0, Math.PI * 2); ctx.fill();
    ctx.beginPath(); ctx.arc(dbX + dbW + 10, dbY + dbH / 2, pulse, 0, Math.PI * 2); ctx.fill();
    // DJ label
    ctx.fillStyle = '#fff';
    ctx.font = 'bold 10px monospace';
    ctx.textAlign = 'center';
    ctx.fillText('DJ', W / 2, dbY + 20);
    // PHOTO WALL — left side, posters
    ctx.fillStyle = '#444';
    ctx.fillRect(22, 80, 60, 30);
    ctx.fillRect(22, 130, 60, 30);
    ctx.fillRect(22, 180, 60, 30);
    ctx.strokeStyle = '#ff0033';
    ctx.lineWidth = 1.5;
    ctx.strokeRect(22, 80, 60, 30);
    ctx.strokeRect(22, 130, 60, 30);
    ctx.strokeRect(22, 180, 60, 30);
    // Posters art (simple shapes)
    ctx.fillStyle = '#ff0033'; ctx.fillRect(30, 85, 10, 20);
    ctx.fillStyle = '#ffcc00'; ctx.fillRect(45, 88, 12, 14);
    ctx.fillStyle = '#00ff66'; ctx.fillRect(30, 135, 44, 20);
    ctx.fillStyle = '#aa00ff'; ctx.fillRect(35, 185, 30, 20);
    // Border walls
    ctx.fillStyle = '#330011';
  ...
  function enterLobby(lobby) {
    ...
    players = [];
    chatLog = [];
    // Switch music to fameup theme (lobby vibe)
    try {
    ...
    resizeHqCanvas();
    if (!enterLobby._resizeWired) {
      enterLobby._resizeWired = true;
      window.addEventListener('resize', resizeHqCanvas);
      window.addEventListener('orientationchange', () => setTimeout(resizeHqCanvas, 100));
    }
    attachInput();
    running = true;
    lastT = 0;
