...
const Sprites = (() => {
  ...
  function drawPlayer(ctx, x, y, opts = {}, dir = 0, frame = 0) {
    ...
    let torsoRows;
    if (body === 'female') {
      // Long hair sides: replace face row padding with hair tiles for female
      const HC = hatColor || '#3a1a1a';
      // Modify face rows to add hair on the sides (use 'h' for hair, separate from hat H)
      ...
      // (palette 'h' will be set below to dark hair color)
      if (shirtless) {
        torsoRows = [
