...
const Sprites = (() => {
  ...
  function drawFan(ctx, x, y, frame = 0, hurtFlash = false, variant = 'flasher', posing = false) {
    // Posing flashers a single bright moment, flashers periodically
    ...
    const palette = {
      ...
      H: '#5a2222',
      D: '#cc4499',
      P: variant === 'flasher' ? '#e94078' : '#ff8800',
      M: variant === 'flasher' ? '#ff66aa' : '#ffaa44',
      C: '#000',
      L: (flash || poseFlash) ? '#ffffff' : '#888',
      V: '#444',
      B: '#000',
      G: '#ffcc00',
      F: '#1a1a1a',
    ...
    // Larger pixel size — 3 instead of 2, so they actually pop
    drawGrid(ctx, x, y, grid, palette, 3);

    if (flash || poseFlash) {
      // Posers' flash is bigger/whiter — like an actual photo flash
      const radius = poseFlash ? 36 : 22;
      const alpha = poseFlash ? 0.85 : 0.6;
      ctx.fillStyle = `rgba(255,${variant === 'flasher' ? 200 : 240},${variant === 'flasher' ? 220 : 240},${alpha})`;
      ctx.beginPath(); ctx.arc(x, y - 4, radius, 0, Math.PI * 2); ctx.fill();
    }
    // Heart above so player knows not to shoot
  ...
  function drawFollower(ctx, x, y, frame = 0, dir = 0, color = '#ff66aa') {
    const palette = {
      H: '#5a2222',
      S: '#e0b58b',
      D: color,
