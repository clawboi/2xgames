...
(function () {
  ...
  function showOnly(id) {
    ...
    // Hide version pill on non-FAMEUP screens (JS fallback for browsers without :has() support)
    const pill = document.getElementById('fu-version-pill');
    if (pill) {
      const hideOn = ['game-screen', 'standoff-game', 'hq-room', 'shop-screen', 'inventory-screen', 'scores-screen', 'start-screen', 'gameover-overlay'];
      pill.style.display = hideOn.includes(id) ? 'none' : '';
    }
    ...
    // Music per screen — FORCE-STOP everything when entering FAMEUP-managed screens
    // so Crabcage menu/gameplay music can't leak into FAMEUP/HQ/Standoff/Vault/Customize
    try {
      if (id in SCREEN_MUSIC) {
        if (window.Audio && Audio.stopMusic) Audio.stopMusic();
        const track = SCREEN_MUSIC[id];
        // playMusic will safely no-op if track failed to load (fameuptheme.mp3 missing)
  ...
  function initVersionPill() {
    ...
    pill.addEventListener('click', async () => {
      if (!confirm('Force update? This will clear all caches and reload.')) return;
      try {
        ...
        // Clear all migration flags so next visit migrates fresh
        Object.keys(localStorage).forEach(k => { if (k.startsWith('fameup_seen_')) localStorage.removeItem(k); });
      } catch (e) {}
  ...
  function initTicker() {
    const track = document.getElementById('fu-ticker-track');
    if (!track) return;
    const names = ['XX7', 'GHOSTLY', 'DRIP', 'STACK', 'BLEND', 'CHASE', 'NOIR', 'YUNG_K', 'OPI', 'SAVAGE', 'TINY', 'MIDNIGHT', 'FAME', 'AURA'];
    const events = [
      n => `<span class="fu-ticker-item">🔥 <span class="name">${n}</span> hit a <span class="stat">QUAD KILL</span> in STANDOFF</span>`,
      n => `<span class="fu-ticker-item">💀 <span class="name">${n}</span> got a <span class="stat">10-STREAK NUKE</span></span>`,
      n => `<span class="fu-ticker-item">🏆 <span class="name">${n}</span> won <span class="stat">3 matches</span> in a row</span>`,
      n => `<span class="fu-ticker-item">🦀 <span class="name">${n}</span> beat <span class="stat">CRABCAGE wave 30</span></span>`,
      n => `<span class="fu-ticker-item">⚡ <span class="name">${n}</span> hit <span class="stat">level ${5 + Math.floor(Math.random() * 30)}</span></span>`,
      n => `<span class="fu-ticker-item">🎯 <span class="name">${n}</span> unlocked <span class="stat">FREEZE GUN</span></span>`,
      n => `<span class="fu-ticker-item">🛍 <span class="name">${n}</span> just dropped in <span class="stat">HQ</span></span>`,
      n => `<span class="fu-ticker-item">💎 <span class="name">${n}</span> collected <span class="stat">all emotes</span></span>`,
      n => `<span class="fu-ticker-item">🎤 <span class="name">2X</span> released <span class="stat">CRABCAGE 2X</span> — featured this drop</span>`,
    ];
    const items = [];
    // Build 16 random items
    for (let i = 0; i < 16; i++) {
