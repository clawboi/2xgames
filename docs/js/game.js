...
(() => {
  ...
  const EP_LINKS = {
    spotify: 'https://open.spotify.com/artist/2x',   // <-- REPLACE WITH REAL SPOTIFY URL
    apple:   'https://music.apple.com/artist/2x',    // <-- REPLACE WITH REAL APPLE MUSIC URL
  };
  //
  ...
  const SONGS = {
    gameplay: 'GAMEPLAY',   // <-- maps to /music/gameplay.mp3   (your main run track)
    boss:     'BOSS',       // <-- maps to /music/boss.mp3       (your aggressive track)
    menu:     'MENU',       // <-- maps to /music/menu.mp3       (your chill intro)
    party:    'AFTERPARTY', // <-- maps to /music/partysong.mp3  (the party scene)
    random:   'RANDOM',     // <-- leave this — rotates the above each wave
  };
  // ════════════════════════════════════════════════════════════════
  ...
  // ============ iOS INSTALL GATE (first-visit on Safari) ============
  // On iPhone Safari (not running as installed PWA), force users to add to
  // home screen. The whole game UI is gated until they re-open from the icon.
  (function showIOSGate() {
    const isIOS = /iPhone|iPad|iPod/.test(navigator.userAgent);
    const isStandalone = window.navigator.standalone === true ||
      window.matchMedia('(display-mode: standalone)').matches;
    if (isIOS && !isStandalone) {
      const gate = document.getElementById('ios-gate');
  ...
  const shopItems = [
    ...
    { key: 'schizo', kind: 'consumable', name: 'ROADIE', desc: 'A homie follows you 3 rounds, shoots crabs', cost: 900 },
  ...
  function startWave(n) {
    ...
    game.timeScaleEnd = 0;
    game.hitstopUntil = 0;
    // If user picked RANDOM track, re-roll on every wave (skipping boss waves where boss music plays)
    if (save.gameplayTrack === 'random' && !Waves.isBossWave(n)) {
      const choices = ['gameplay', 'boss', 'menu', 'party'];
      const pick = choices[Math.floor(Math.random() * choices.length)];
      try {
        const el = document.getElementById('music-' + pick);
        if (el && (!Audio._currentTrack || Audio._currentTrack !== pick)) {
          // Use the wrapped playMusic which already handles random
        }
      } catch (e) {}
    }
  ...
  function setGameOver(victory) {
    ...
    if (!victory) {
      game.slowMo(1400, 0.2);
      game.deathFade = 0;  // animates up in main loop
      try { game.spawnFloater(game.player.x, game.player.y - 30, 'DROPPED', '#ff0033', 32, -0.5); } catch (e) {}
      try { Audio.sfx.gameOver(); } catch (e) {}
      try { Audio.fadeOutMusic(1100); } catch (e) {}
    } else {
      game.slowMo(2000, 0.3);
    }
