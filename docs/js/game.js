...
(() => {
  ...
  refreshHighScoreUI();
  window._refreshHighScoreUI = refreshHighScoreUI;
  ...
  // PRESETS — module-level so both start screen + coop lobby can use them
  const PRESETS = {
  ...
  function firstInteractionUnlock() {
    Audio.unlock();
    if (game.state === 'menu' && isStartScreenVisible()) Audio.playMusic('menu');
  }
  ...
  function initCustomizer() {
    ...
    // PRESETS — one-tap complete looks (uses module-level PRESETS)
  ...
  function renderLobbySlots() {
    ...
    for (let s = 0; s < 4; s++) {
      ...
      html += `<div style="display:flex;align-items:center;gap:8px;padding:6px 10px;margin:4px 0;background:rgba(0,0,0,0.4);border-left:4px solid ${tints[s]};">
        <div style="color:${tints[s]};font-size:11px;width:36px;">P${s+1}</div>
        <canvas class="lobby-slot-preview" data-slot="${s}" width="36" height="48" style="background:#000;border:1px solid ${tints[s]};border-radius:3px;image-rendering:pixelated;"></canvas>
        <div style="flex:1;font-size:10px;color:#aaa;">${(ls.customization && ls.customization.preset) || 'no pick'} · ${ls.difficulty.toUpperCase()}</div>
        <div style="font-size:9px;">${status}</div>
      </div>`;
    }
    coopSlotsContainer.innerHTML = html;
    // Now draw each preview into its canvas
    coopSlotsContainer.querySelectorAll('.lobby-slot-preview').forEach(cv => {
      const s = parseInt(cv.getAttribute('data-slot'));
      const ls = lobbyState[s];
      const ctx = cv.getContext('2d');
      ctx.imageSmoothingEnabled = false;
      ctx.fillStyle = '#000'; ctx.fillRect(0, 0, cv.width, cv.height);
      if (ls && ls.connected && ls.customization) {
        try { Sprites.drawPlayer(ctx, cv.width / 2, cv.height / 2 + 8, ls.customization, 'down', 0); } catch (e) {}
      }
    });
  }
  ...
  function handleCoopMessage(msg, fromSlot) {
    ...
    switch (msg.t) {
      ...
      case 'boss_intro':
        if (Coop.isGuest() && window._game) {
          const b = msg.d || {};
          // Show the actual boss cutscene on guest too
          if (typeof showBossCutscene === 'function') {
            showBossCutscene(b.type || 'mirror2x', b.name, () => {});
          } else {
      ...
      case 'victory':
        ...
        break;
      case 'music':
        // Host changed music track — guest plays it too + updates UI
        if (Coop.isGuest() && msg.d && msg.d.track) {
          if (msg.d.track === 'stop') {
            try { Audio.stopMusic(); } catch (e) {}
            if (typeof renderAudioPlayer === 'function') {
              const w = document.getElementById('audio-player');
              if (w) w.classList.remove('playing');
            }
          } else {
            try { Audio.playMusic(msg.d.track); } catch (e) {}
            // Update guest's audio player display to match host's track
            if (typeof PLAYLIST_DEFS !== 'undefined' && typeof renderAudioPlayer === 'function') {
              const idx = PLAYLIST_DEFS.findIndex(t => t.key === msg.d.track);
              if (idx >= 0) {
                playerCurrentIdx = idx;
  ...
  function drawCoopPreview() {
    ...
    const slotTint = tints[slot];
    // Show actual outfit colors — don't override fit, let preset come through
    try { Sprites.drawPlayer(ctx, cv.width / 2, cv.height / 2 + 20, customization, 'down', 0); } catch (e) {}
    // Slot color border around the canvas (drawn on top inside the canvas)
    ctx.strokeStyle = slotTint;
    ctx.lineWidth = 3;
    ctx.strokeRect(1.5, 1.5, cv.width - 3, cv.height - 3);
    // Slot label
    ctx.font = 'bold 9px "Press Start 2P", monospace';
    ctx.fillStyle = slotTint; ctx.textAlign = 'center';
    ctx.fillText('P' + (slot + 1) + ' (YOU)', cv.width / 2, cv.height - 8);
  }
  ...
  const shopItems = [
    ...
    { key: 'pwr_timewarp', kind: 'power', name: '⌛ TIME WARP', desc: 'Q for 6s of slo-mo', cost: 350 },
  ...
  function renderShop() {
    ...
    wrap.querySelectorAll('.shop-buy:not(.maxed)').forEach(btn => {
      // Track touch start to differentiate scroll from tap
      ...
      btn.addEventListener('click', (e) => {
        // If user scrolled while touching this button, do NOT buy
        if (scrollDetected) {
          e.preventDefault();
          e.stopPropagation();
          scrollDetected = false;
  ...
  function resetGame() {
    ...
    // Hide spectator overlay (in case of restart from spectating)
    const sp = document.getElementById('spectator-overlay');
    if (sp) sp.classList.add('hidden');
    // Hide boss cutscene if it was stuck
    const bc = document.getElementById('boss-cutscene');
  ...
  function showBossCutscene(type, bossName, onDone) {
    ...
    // FREEZE the world during the 2.4s cutscene so enemies can't hit you
    game.hitstopUntil = performance.now() + 2500;
  ...
  function spawnBoss(type, hpMult) {
    ...
    const doSpawn = () => {
      ...
      if (game.coopActive && game.coopRole === 'host') {
        Coop.broadcast('boss_intro', { name: boss.name, type: cutsceneType });
      }
    };
  ...
  function loop(now) {
    try {
      ...
      if (game.timeScale < 1 && now > game.timeScaleEnd) {
        // Smooth ease-back to 1.0 — distance-based so it eases out naturally
        game.timeScale = Math.min(1, game.timeScale + (1 - game.timeScale) * 0.08 + 0.02);
      }
      const scaledDt = dt * game.timeScale;
      if (game.state === 'playing' || game.state === 'cd-wait' || game.state === 'party' || game.state === 'spectating') {
  ...
  function updateGuestFromHostState() {
    ...
    // Rebuild enemies + bullets as render proxies — render correct sprite per type
    game.enemies = (hs.enemies || []).map(e => ({
      x: e.x, y: e.y, hp: e.h, maxHp: e.mh, radius: e.r,
      isBoss: e.b, frozen: e.fr, dead: false, type: e.t,
      _frame: 0,
      draw(ctx) {
        this._frame = (this._frame + 1) % 256;
        try {
          const hpFrac = this.hp / this.maxHp;
          // Render with type-appropriate sprite
          if (this.type === 'GiantCrab') {
            Sprites.drawGiantCrab(ctx, this.x, this.y, this._frame, hpFrac, false);
          } else if (this.type === 'Slimey') {
            Sprites.drawSlimey(ctx, this.x, this.y, this._frame, hpFrac, false);
          } else if (this.type === 'Mirror2X') {
            Sprites.drawMirror2X(ctx, this.x, this.y, customization, this._frame, hpFrac, false);
          } else if (this.type === 'FastCrab') {
            Sprites.drawCrab(ctx, this.x, this.y, this._frame, hpFrac, false, 0.9, '#ff6644');
          } else if (this.type === 'TankCrab') {
            Sprites.drawCrab(ctx, this.x, this.y, this._frame, hpFrac, false, 1.3, '#660033');
          } else {
            Sprites.drawCrab(ctx, this.x, this.y, this._frame, hpFrac, false);
          }
          if (this.frozen) {
            ctx.fillStyle = 'rgba(136,221,255,0.45)';
            ctx.beginPath(); ctx.arc(this.x, this.y, this.radius + 3, 0, Math.PI * 2); ctx.fill();
          }
          // Boss HP bar — large enemies need a visible health bar
          if (this.isBoss) {
            const barW = 80;
            ctx.fillStyle = '#000'; ctx.fillRect(this.x - barW/2 - 1, this.y - this.radius - 14, barW + 2, 6);
            ctx.fillStyle = '#ff0033'; ctx.fillRect(this.x - barW/2, this.y - this.radius - 13, barW * (this.hp / this.maxHp), 4);
          }
        } catch (er) {}
      }
    }));
    game.bullets = (hs.bullets || []).map(b => ({
      ...
      draw(ctx) {
        if (b.s) {
          // Player bullet — orange-yellow
          ctx.fillStyle = '#ffaa00';
          ctx.beginPath(); ctx.arc(this.x, this.y, 3, 0, Math.PI * 2); ctx.fill();
        } else {
          // Enemy bullet — red, slightly bigger so they read as threats
  ...
  function update(dt) {
    ...
    if (game.coopActive && game.coopRole === 'guest') {
      ...
      // Check if my player just died → enter spectator mode
      if (game.player && game.player.dead && game.state === 'playing') {
        enterSpectatorMode();
      }
      // Check if I was revived by the host (hp restored externally)
      if (game.state === 'spectating' && game.player && !game.player.dead && game.player.hp > 0) {
        exitSpectatorMode();
      }
      return;
    }
    // COOP HOST: check for all-dead → trigger real gameover
    if (game.coopActive && game.coopRole === 'host' && game.state !== 'gameover' && game.state !== 'win') {
      const localDown = !game.player || game.player.dead;
      const partnersDown = !(game.partners || []).some(p => p && !p.dead);
      if (localDown && partnersDown && (game.partners || []).filter(Boolean).length > 0) {
        // Everyone's down — wipe
        setGameOver(false);
      }
    }
  ...
  function updateHUD() {
    if (!game.player) return;
    // Update spectator UI bank while in spectator mode
    if (game.state === 'spectating') {
      try { updateSpectatorUI(); } catch (e) {}
    }
  ...
  window.addEventListener('keydown', e => {
    ...
    if (e.key === 'Tab') {
      e.preventDefault();
      if (game.state === 'playing' || game.state === 'paused' || game.state === 'party' || game.state === 'cd-wait' || game.state === 'spectating') togglePause();
    }
  ...
  function togglePause(fromNetwork) {
    ...
    if (game.state === 'playing' || game.state === 'party' || game.state === 'cd-wait' || game.state === 'spectating') {
      game._prePauseState = game.state;
      game.state = 'paused';
      document.body.classList.remove('playing');
      pauseOverlay.classList.remove('hidden');
  ...
  function enterSpectatorMode() {
    ...
    // Local player visually drops + faded
    if (game.player) {
      game.player.dead = true;
      game.player.deathTilt = (Math.random() - 0.5) * 1.4;
      game.player.gunLockEnd = performance.now() + 99999;
    }
    try { Audio.sfx.gameOver(); } catch (e) {}
    try { Audio.haptic.death(); } catch (e) {}
    try { game.spawnFloater(game.player.x, game.player.y - 30, 'DOWNED', '#ff0033', 28, -0.4); } catch (e) {}
    try { game.spawnExplosion(game.player.x, game.player.y, '#ff0033', 18); } catch (e) {}
    // Show overlay
  ...
  function exitSpectatorMode() {
    ...
    if (game.player) {
      ...
      game.player.deathTilt = 0;
      game.player.gunLockEnd = 0;
    }
    const ov = document.getElementById('spectator-overlay');
    if (ov) ov.classList.add('hidden');
    try { game.spawnFloater(game.player.x, game.player.y - 30, 'REVIVED', '#00ff66', 24, -0.4); } catch (e) {}
    try { Audio.haptic.heavy(); } catch (e) {}
  }

  function updateSpectatorUI() {
    const btn = document.getElementById('spec-revive-btn');
    const info = document.getElementById('spec-revive-info');
    if (!btn || !info) return;
    const bank = save.cashBank || 0;
    info.textContent = `YOUR BANK: $${bank}`;
    if (bank >= 4200) {
      btn.classList.remove('broke');
      btn.disabled = false;
    } else {
      btn.classList.add('broke');
      btn.disabled = true;
    }
  }

  // Wire the floating revive button
  const specReviveBtn = document.getElementById('spec-revive-btn');
  if (specReviveBtn) {
    wireTap(specReviveBtn, () => {
      if ((save.cashBank || 0) < 4200) return;
      save.cashBank -= 4200;
      persist();
      exitSpectatorMode();
      // Tell partners we're back (host echoes to all guests)
      if (Coop.isGuest()) {
        Coop.send('revive', { by: 'self' });
      } else if (Coop.isHost()) {
        Coop.broadcast('revive_done', { slot: 0 });
      }
    });
  }

  function setGameOver(victory) {
    if (game.state === 'gameover' || game.state === 'win' || game.state === 'spectating') return;
    // COOP: when LOCAL player dies, enter spectator mode if partners are still alive
    if (!victory && game.coopActive) {
      const livingPartners = (game.partners || []).filter(p => p && !p.dead).length;
      if (livingPartners > 0) {
        enterSpectatorMode();
        return;
      }
      // No partners alive → real gameover continues below
    }
  ...
  document.body.addEventListener('touchmove', e => {
    ...
    // Allow scrolling on FAMEUP-managed screens
    if (e.target.closest('#fameup-home')) return;
    if (e.target.closest('#vault-screen')) return;
    if (e.target.closest('#customize-screen')) return;
    if (e.target.closest('#standoff-lobby')) return;
    if (e.target.closest('#hq-lobby')) return;
    if (e.target.closest('#settings-overlay')) return;
    if (e.target.closest('.overlay-content')) return;
    if (e.target.closest('#hq-chat-log')) return;
    if (e.target.closest('.fu-ticker')) return;
    e.preventDefault();
  }, { passive: false });
  document.addEventListener('gesturestart', e => e.preventDefault());

  // Expose game globally so cross-file modules (entities.js, etc) can access game.timeScale for slo-mo
  window.game = game;
})();
