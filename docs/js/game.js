...
(() => {
  ...
  // ============ iOS INSTALL GATE — decides which screen to show on load ============
  (function showIOSGate() {
    ...
    if (isIOS && !isStandalone) {
      // iOS Safari, not yet installed → show install gate, hide everything else
      if (gate) gate.classList.remove('hidden');
      if (start) start.classList.add('hidden');
    } else {
      // Desktop, Android, or installed iOS PWA → show normal start screen
  ...
  function updateHUD() {
    ...
    if (powEl) {
      ...
      if (ps.length > 0) {
        const icons = { pwr_shockwave:'⚡', pwr_nuke:'☢', pwr_heal:'✚', pwr_timewarp:'⌛', pwr_berserk:'🔥' };
  ...
  function togglePause() {
    if (game.state === 'playing' || game.state === 'party' || game.state === 'cd-wait') {
      ...
      if (inv) {
        ...
        const ps = save.powers || [];
        if (ps.length) {
          const icons = { pwr_shockwave:'⚡SHOCK', pwr_nuke:'☢NUKE', pwr_heal:'✚HEAL', pwr_timewarp:'⌛WARP', pwr_berserk:'🔥BERSERK' };
