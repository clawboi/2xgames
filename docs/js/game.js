...
(() => {
  ...
  function openCoopLobby(role) {
    ...
    if (role === 'host') {
      ...
      Coop.host((code) => {
        ...
        // Host always sees lobby list too (with own slot)
    ...
    Coop.onConnect((slot, totalConnections) => {
      // Both host and guest land here
      if (Coop.isHost()) {
        // New guest connected — show in lobby
        lobbyState[slot].connected = true;
        renderLobbySlots();
        // Send the new guest the current lobby snapshot
        Coop.sendTo(slot, 'lobby_sync', { slot, lobby: lobbyState });
      } else {
        // I'm the guest — host accepted me
        coopJoinView.classList.add('hidden');
        coopLobbyList.classList.remove('hidden');
        // Lock the difficulty buttons — only host decides
    ...
    Coop.onDisconnect((slot) => {
      ...
      if (game.coopActive && (game.state === 'playing' || game.state === 'paused')) {
        // Mid-game disconnect — just mark them inactive
        if (game[`player${slot + 1}`]) game[`player${slot + 1}`].dead = true;
      }
    });
  }
  ...
  function handleCoopMessage(msg, fromSlot) {
    ...
    switch (msg.t) {
      ...
      case 'revive':
        if (Coop.isHost() && window._game) {
          const slot = fromSlot;
          const p = window._game[`player${slot + 1}`];
  ...
  document.querySelectorAll('.coop-preset').forEach(btn => {
    wireTap(btn, () => {
      ...
      applyPreset(preset);
      // Mark customization with preset name for display
  ...
  wireTap(document.getElementById('coop-ready-btn'), () => {
    ...
    document.getElementById('coop-ready-btn').textContent = 'WAITING...';
  ...
  function spawnBoss(type, hpMult) {
    ...
    // COOP: broadcast boss intro to all guests so they see + hear it too
    if (game.coopActive && game.coopRole === 'host') {
  ...
  document.querySelectorAll('.track-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (game.coopActive && game.coopRole === 'host') {
        try { Coop.broadcast('music', { track }); } catch (e) {}
      }
    });
  });
