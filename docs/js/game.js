...
(() => {
  ...
  const game = {
    ...
    MAX_BULLETS: 250, MAX_PARTICLES: 400, MAX_FLOATERS: 60,
    MAX_DUST: 50, MAX_CASH: 80, MAX_BLOOD: 30,

    findNearestEnemy(x, y) {
      let best = null, bestD = Infinity;
      for (const e of this.enemies) {
        if (e.dead || e.isFan) continue; // skip fans for auto-aim
    ...
    exitParty() {
      ...
      try { Audio.stopPartyBeat(); } catch (e) {}
      // If girls were recruited, they become followers (up to 3)
      this.followers = [];
      if (this.recruitedList && this.recruitedList.length > 0) {
        const colors = ['#ff66aa','#ffaa00','#aa66ff'];
        this.recruitedList.forEach((d, i) => {
          this.followers.push(new Follower(this.player.x - 30 - i * 14, this.player.y, 'Fan'+(i+1), i, colors[i] || '#ff66aa'));
        });
      } else if (this.recruitedDancer) {
        this.followers.push(new Follower(this.player.x - 20, this.player.y, 'Fan', 0, '#ff66aa'));
      }
  ...
  function initCustomizer() {
    ...
    const presets = {
      schizo:    { fit:'#cc0022', accent:'#000000', hat:'durag',      chain:'gold',     shades:true,  pattern:'glow',   shirtless:true,  tattoos:true },
      drip:      { fit:'#ffffff', accent:'#ffcc00', hat:'cap',        chain:'cuban',    shades:true,  pattern:'solid',  shirtless:false, tattoos:false },
      streetwear:{ fit:'#000000', accent:'#cc0022', hat:'hood',       chain:'gold',     shades:false, pattern:'solid',  shirtless:false, tattoos:true },
      rage:      { fit:'#ff6600', accent:'#cc0022', hat:'mohawk',     chain:'none',     shades:false, pattern:'flame',  shirtless:true,  tattoos:true },
      clean:     { fit:'#ffffff', accent:'#000000', hat:'none',       chain:'platinum', shades:true,  pattern:'solid',  shirtless:false, tattoos:false },
      ghost:     { fit:'#9933ff', accent:'#00aaff', hat:'headphones', chain:'ice',      shades:true,  pattern:'glow',   shirtless:true,  tattoos:false },
    };
    ...
    function refreshCustomizerUI() {
      ...
      Object.entries(map).forEach(([k, sel]) => {
        document.querySelectorAll(sel).forEach(el => el.classList.toggle('selected', String(el.dataset[k]) === String(customization[k])));
      });
    }
  ...
  // ============ NAV ============
  document.getElementById('start-btn').addEventListener('click', () => { Audio.unlock(); startGame(); });
  ...
  function renderShop() {
    ...
    for (const item of shopItems) {
      ...
      if (item.kind === 'upgrade') {
        ...
        // Schizo companion — show owned count, can buy multiple
  ...
  function resetGame() {
    ...
    game.truck.maxHp = 200 + (save.upgrades.truckHp * 60);
  ...
  function setBgTint(n) {
    const tints = ['#1a0f08','#1a0f12','#1a121f','#0f1a1a','#1a1a0f','#2a1010','#1f0a2a','#0a1a1f','#2a1a0f','#1a0a1f','#0f1a0a'];
  ...
  function updateWeaponBar() {
    document.querySelectorAll('.weapon-slot').forEach((el, i) => {
      ...
      const isCrabSlot = el.dataset.weapon === '4';
      const isShotgunSlot = el.dataset.weapon === '5';
      const isStunSlot = el.dataset.weapon === '6';
      if (isCrabSlot) {
  ...
  if (emoteBtn) {
    const triggerEmote = () => {
      if (game.state === 'playing' && game.player) {
        game.player.triggerEmote('dance');
      }
    };
    emoteBtn.addEventListener('click', triggerEmote);
    emoteBtn.addEventListener('touchstart', e => { e.preventDefault(); triggerEmote(); }, { passive: false });
  }
  ...
  document.getElementById('pause-shop-btn').addEventListener('click', () => {
    ...
    document.getElementById('shop-back').onclick = () => {
      // Restore unlocks to player in-game
      if (save.unlocks.shotgun) game.player.unlockedWeapons.add(5);
      if (save.unlocks.stungun) game.player.unlockedWeapons.add(6);
      game.player.applyUpgrades(save.upgrades);
      // Apply truck upgrade live
      const newTruckMax = 200 + (save.upgrades.truckHp * 60);
      ...
      // Restore default shop-back handler
      document.getElementById('shop-back').textContent = 'BACK';
      document.getElementById('shop-back').onclick = () => { showScreen('start-screen'); refreshHighScoreUI(); };
      updateWeaponBar();
    };
    pauseOverlay.classList.add('hidden');
