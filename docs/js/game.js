...
(() => {
  ...
  // ============ EP STREAMING LINKS (edit these to your real URLs) ============
  // Replace these URLs with the actual Spotify/Apple Music album links.
  const EP_LINKS = {
    spotify: 'https://open.spotify.com/artist/schizobaby',
    apple:   'https://music.apple.com/artist/schizobaby',
  };
  ...
  (function showIOSGate() {
    ...
    const dismissed = localStorage.getItem('crabcage_install_gate_dismissed');
    if (isIOS && !isStandalone && !dismissed) {
      const gate = document.getElementById('ios-gate');
      if (gate) {
        gate.classList.remove('hidden');
        // Hide start screen behind it
        document.querySelectorAll('.screen').forEach(s => { if (s.id !== 'ios-gate') s.classList.add('hidden'); });
        const cont = document.getElementById('install-gate-continue');
        if (cont) {
          wireTap(cont, () => {
            gate.classList.add('hidden');
            localStorage.setItem('crabcage_install_gate_dismissed', '1');
  ...
  function refreshTrackPickUI() {
    document.querySelectorAll('.track-pick-btn').forEach(b => {
      b.classList.toggle('selected', b.dataset.gameplayTrack === (save.gameplayTrack || 'gameplay'));
    });
  }
  refreshTrackPickUI();
  document.querySelectorAll('.track-pick-btn').forEach(b => {
  ...
  function setGameOver(victory) {
    ...
    let statsHTML = `WAVE ${game.wave} · SCORE ${game.score}<br>CASH $${game.cashCollected} BANKED<br>BANK $${save.cashBank} · HIGH ${save.highScore}`;
