...
(() => {
  ...
  wireTap(document.getElementById('shop-back'), () => { showScreen('start-screen'); refreshHighScoreUI(); refreshTrackRowVisibility(); });
  ...
  function update(dt) {
    ...
    // Dynamic boss music — speed up as boss HP drops
    if (game.bossActive) {
      const boss = game.enemies.find(e => e.isBoss && !e.dead);
      if (boss) {
        const hpPct = boss.hp / boss.maxHp;
        // 1.0 at full HP, up to 1.25 near death
        const rate = 1.0 + (1 - hpPct) * 0.25;
        const bossEl = document.getElementById('music-boss');
        if (bossEl && !bossEl.paused) {
          try { bossEl.playbackRate = rate; } catch (e) {}
        }
      }
    }
    updateWaves(dt);
