...
(() => {
  ...
  // Prevent iOS gestures / pinch — ONLY on gameplay elements, NOT globally
  // (body-wide passive:false listener disables iOS native scroll optimization on ALL scrollable children)
  const blockTouch = (e) => e.preventDefault();
  ['game-canvas', 'joystick', 'touch-actions'].forEach(id => {
    const el = document.getElementById(id);
    if (el) el.addEventListener('touchmove', blockTouch, { passive: false });
  });
