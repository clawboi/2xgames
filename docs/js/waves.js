...
const Waves = (() => {
  ...
  function getWaveConfig(waveNum) {
    ...
    const total = Math.min(6 + waveNum * 1.6, 38); // less crowded — was 8 + waveNum*2, max 50
    ...
    if (waveNum >= 10) { fan = 0.08; paparazzi = 0.04; }    // fans rarer now
    if (waveNum >= 11) { tank = 0.12; exploder = 0.12; fast = 0.14; armed = 0.14; }
    if (waveNum >= 12) summoner = 0.08;
    if (waveNum >= 13) { tank = 0.14; exploder = 0.14; armed = 0.16; fan = 0.10; leaper = 0.12; }
    // Post-party (wave 16+) — explicitly calm down, less chaos
    if (waveNum >= 16) {
      fan = 0.08; paparazzi = 0.04;
      summoner = 0.08;
    }
    ...
    return {
      ...
      spawnInterval: Math.max(420, 1100 - waveNum * 30), // slower spawns — was 260/900/-32
    };
  }
