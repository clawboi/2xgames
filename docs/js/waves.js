...
const Waves = (() => {
  ...
  function getWaveConfig(waveNum) {
    ...
    const mod = Math.min(1.5, 1 + (waveNum - 1) * 0.03); // 10% HP per wave, max 240%
    const total = Math.min(8 + waveNum * 2, 50);

    // Spawn probabilities — sum can be up to ~0.85, remainder = regular crabs
    ...
    if (waveNum >= 8)  armed = 0.13;
    if (waveNum >= 9)  leaper = 0.10;           // NEW: leaper crabs
    if (waveNum >= 10) { fan = 0.13; paparazzi = 0.05; }
    if (waveNum >= 11) { tank = 0.12; exploder = 0.12; fast = 0.16; armed = 0.16; }
    if (waveNum >= 12) summoner = 0.10;         // NEW: summoner crabs
    if (waveNum >= 13) { tank = 0.14; exploder = 0.14; armed = 0.18; fan = 0.16; leaper = 0.13; }
    if (waveNum >= 16) summoner = 0.14;
