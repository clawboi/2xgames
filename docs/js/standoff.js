...
(function () {
  ...
  function getPlayerInput() {
    ...
    // Aim direction:
    // - Touch + twin-stick (drag from fire btn): face the drag direction
    // - Touch + actively moving (no aim drag): face movement direction
    ...
    if (player) {
      ...
      if (touchAim != null) {
        // Twin-stick aim: drag from fire button overrides everything
  ...
  function update(dt) {
    ...
    // Pickups
    pickupSpawnTimer -= dt;
    if (pickupSpawnTimer <= 0 && pickups.length < 5) {
      pickupSpawnTimer = 6000 + Math.random() * 4000;
      spawnPickup();
    }
    for (const pk of pickups) {
      pk.life -= dt;
