...
(function () {
  ...
  function resizeStdCanvas() {
    if (!canvas) return;
    // Match canvas internal resolution to viewport for crisp rendering
    const w = window.innerWidth;
    const h = window.innerHeight;
    canvas.width = w;
    canvas.height = h;
    W = w; H = h;
    // Rebuild current map at new size if a match is active
    if (running && obstacles && obstacles.length) {
      // Don't rebuild during active match - just clamp coords
    }
  }
  let entities = [];
  let bullets = [];
  ...
  function rect(x, y, w, h, color, opts) {
    opts = opts || {};
    // Scale from reference 800×600 to actual viewport
    const sx = W / 800;
    const sy = H / 600;
    const rx = x * sx;
    const ry = y * sy;
  ...
  function getPlayerInput() {
    ...
    // Aim direction:
    // - Touch device + actively moving: face movement direction (shoot where you're going)
    // - Touch device + not moving: keep last facing
    // - Desktop: face mouse cursor
    if (player) {
      const isTouch = touchMove.active || ('ontouchstart' in window && !mouseDown);
      if (isTouch && touchMove.active && (Math.abs(x) > 0.05 || Math.abs(y) > 0.05)) {
        player.facing = Math.atan2(y, x);
      } else if (!isTouch) {
        player.facing = Math.atan2(mouseY - player.y, mouseX - player.x);
      }
      // If touch & not moving: keep current facing (don't twitch)
    }
    return {
      x, y,
  ...
  function render() {
    ...
    // Pickups
    for (const pk of pickups) {
      const c = pk.type === 'health' ? '#00ff66' : pk.type === 'damage' ? '#ff6600' : '#ffcc00';
      ctx.fillStyle = c;
  ...
  function openLobby() {
    ...
    // Mobile fire button — main action, hold to fire continuously
    const fireBtn = document.getElementById('std-fire-btn');
    if (fireBtn) {
      const press = (e) => { if (e) e.preventDefault(); touchFire = true; };
