...
const Audio = (() => {
  ...
  function playMusic(name) {
    ...
    // ALWAYS stop ALL other tracks first — even if our target track is broken,
    // we don't want stale music (e.g. Crabcage menu) leaking into other screens
    Object.values(tracks).forEach(other => {
      if (other && other !== t) {
        try { other.pause(); other.currentTime = 0; other.volume = 0; } catch (e) {}
      }
    });
    // If target track previously errored, stay silent (don't fall back to wrong music)
    if (t._loadFailed) {
      currentTrack = null;
  ...
  function stopMusic() {
    // Pause ALL tracks — Crabcage menu may have started a track outside the tracking
    Object.values(tracks).forEach(t => {
      if (t) {
        try { t.pause(); t.currentTime = 0; t.volume = 0; } catch (e) {}
      }
    });
    currentTrack = null;
    currentTrackName = null;
